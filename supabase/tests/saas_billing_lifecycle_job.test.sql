...
-- `started_at = now()` scopes this to this test's own invocation: this job is
-- also pg_cron-scheduled inside the container the suite runs against, so an
-- unqualified count can pick up a concurrently-committed cron-fired row.
-- Works because `now()` is the transaction timestamp and the function records
-- `started_at := now()` -- see the fuller explanation in
-- check_out_manual_auto_timeout.test.sql, where this raced in real CI.
select is(
  (select count(*) from job_runs
    where job_name = 'saas_billing_lifecycle' and status = 'success' and started_at = now())::int, 1,
