...
-- fixture rows are seeded up front as the connecting role, same convention
-- as every other RLS test file -- actor Y (no escalation) stands in for the
-- "before escalation" case instead of toggling one session's own state
-- mid-transaction.
--
-- Story 1.15 update: the grant is no longer the audit_log row. 0085 moved it
-- to the gym_data_escalations table so it can carry a 24-hour TTL and be
-- revoked, so this file's fixture now seeds BOTH -- the grant row (which is
-- what the rewritten members/payments policies actually read) and the
-- audit_log row (which is what the Audit trail tab reads, and what the two
-- assertions at the bottom of this file are about). Everything else here is
-- unchanged: these per-actor and per-gym isolation assertions are still
-- exactly the right ones, and they now exercise the new predicate.
-- The TTL/revocation lifecycle itself is covered by
-- gym_data_escalation_ttl_revocation.test.sql, not here.
...
-- The escalation grant itself (Story 1.15): a live, unexpired, unrevoked
-- gym_data_escalations row for (Gym A, actor X), simulating what
-- escalate_gym_data_access() would have written. Before 0085 this was the
-- audit_log row below; it is now this row that the members/payments
-- policies read.
insert into gym_data_escalations (id, gym_id, actor_id, reason, expires_at)
values (
  '00000000-0000-0000-0000-000000003051',
...
-- The accountability record for that same escalation, written in the same
-- transaction by escalate_gym_data_access() in production. It no longer
-- grants anything -- it is here because the two audit_log read-policy
-- assertions at the bottom of this file need a row to see, and because
-- keeping the pair together documents the post-0085 shape (grant row +
-- audit row, target_entity_id pointing at the grant).
