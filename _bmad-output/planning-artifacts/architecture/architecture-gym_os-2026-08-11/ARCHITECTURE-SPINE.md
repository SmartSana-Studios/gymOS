...
- **OQ-7 remains genuinely open** (`prd.md` Open Questions table still lists it as blocking "full production reliance on Tara Money" and citing Flow B, even though OQ-14 itself is separately marked resolved — a residual PRD-internal inconsistency worth flagging back to `bmad-prd`, not fixed here) — re-verifying the sandbox spike against GymOS's real activated business account (`9FmIZg9GBB`), swapping off the Temporal stand-in. This is an account-credential swap, not a data-model question; it doesn't block any AD above, since AD-14/AD-15's credential-selection mechanism is account-agnostic.
