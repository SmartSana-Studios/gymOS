...
**Review round 1 (branch `bmad/6.1-expo-push-fixes` @ `55eda46`, diff vs `master`) — 2026-08-04:**

- [x] [Review][Patch] `run_subscription_lifecycle_job()`'s three `perform private.send_push_notification(...)` calls have no per-call exception isolation, unlike Story 6.3's payments trigger in the same diff, which deliberately wraps each notification call so a notification failure cannot roll back the real write it rides on. Here, a single notification-dispatch exception propagates to the job's one outer exception handler, rolling back every subscription status transition (`expired`/`grace_period`/`expiring_soon`) already committed in that run, not just the notification. Fixed: each `perform` call is now wrapped in its own `begin...exception when others...raise warning...end` block. [supabase/migrations/0045_subscription_lifecycle_notifications.sql:394,415,428]
- [x] [Review][Patch] `private.send_push_notification()`'s per-token dispatch loop has no exception isolation between iterations — a `net.http_post`/insert failure on one device token rolls back the dispatch row and any already-queued delivery rows for other tokens on the same event. Because the idempotency key is gone after rollback and nothing retries a job that already ran, the notification is then permanently lost for every device on that event, not just the failing token. Fixed: each loop iteration is now wrapped in its own exception-isolated block. [supabase/migrations/0045_subscription_lifecycle_notifications.sql:145-178]
- [x] [Review][Patch] `notification_delivery_processor` (every-minute cron) has no protection against overlapping invocations. If one run takes longer than a minute (Expo slowness, backlog growth), pg_cron can start a second overlapping run, and two backends can process/mutate the same delivery row concurrently, double-firing Expo requests. Fixed: added a `pg_try_advisory_xact_lock()` guard at the top of `process_notification_deliveries()` so an overlapping invocation returns immediately instead of processing the same rows (row-level `FOR UPDATE SKIP LOCKED` was tried first but Postgres rejects `FOR UPDATE` on a query combined with `UNION`, which this function uses once extended by 0046). [supabase/migrations/0045_subscription_lifecycle_notifications.sql, supabase/migrations/0046_payment_notifications.sql — private.process_notification_deliveries]
- [x] [Review][Defer] `private.notification_dispatches`/`private.notification_deliveries` have no retention/purge job and grow unbounded — deferred, pre-existing: matches every other unbounded-growth table already accepted in this codebase (`job_runs`, the audit log, `otp_resend_attempts`). [supabase/migrations/0045_subscription_lifecycle_notifications.sql]
- [x] [Review][Defer] `idx_notification_deliveries_due`'s `(status, updated_at)` shape doesn't match `process_notification_deliveries()`'s actual `order by created_at, id` — deferred, real but low-cost (index-size only) and speculative until an actual staleness/stuck-delivery consumer is built. [supabase/migrations/0045_subscription_lifecycle_notifications.sql]
- [x] [Review][Defer] A `pg_net` response row garbage-collected before the once-a-minute processor reads it would leave that delivery permanently stuck at `push_pending`/`receipt_pending` with no terminal state — deferred: fixing this requires choosing a staleness policy/threshold (not an unambiguous patch), and is low-likelihood given pg_net's default retention window versus minute-by-minute polling. [supabase/migrations/0045_subscription_lifecycle_notifications.sql — private.process_notification_deliveries]
- [x] [Review][Defer] N-01/N-02's exact-date/no-backfill design means a missed or failed nightly cron run permanently drops that subscription's reminder with no compensating alert — deferred, same accepted risk class as Story 3.1's own no-auto-retry job-failure model (`0021_subscription_lifecycle_cron.sql`, already accepted in `deferred-work.md`). [supabase/migrations/0045_subscription_lifecycle_notifications.sql — run_subscription_lifecycle_job]

Dismissed as noise/non-issues (verified against repo state, not new problems):
- `payment-webhook/index.ts`'s blanket non-`"verified"`-is-a-failure branch — already logged in `deferred-work.md` from the Story 6.3 review round; re-raised as a duplicate, not a new finding.
