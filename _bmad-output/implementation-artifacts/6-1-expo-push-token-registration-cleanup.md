...
**Review round 3 (branch `bmad/6.1-expo-push-fixes` @ `222271b`, uncommitted working-tree diff) — 2026-08-04:**

- [x] [Review][Decision] `docs/decisions.md` rewrites Story 6.1's existing 2026-08-03 "Decision 4" entry in place (same date, replaced text) instead of appending a new dated correction, even though the log's own header states "One entry per decision" and that entries record outcomes "that can't be changed later without cost." — resolved: kept consistent with how this same diff already handles the 6.2 material (appended, not edited). Reverted the in-place rewrite; the original 2026-08-03 Decision 4 text stands unchanged with a "Superseded 2026-08-04" pointer, and a new dated entry above it records the actual resolution. [docs/decisions.md:7]
- [x] [Review][Patch] `apps/mobile/app.config.js` — the sole file that replaces the `googleServicesFile` static config removed from `app.json` in this diff — is untracked by git (never added). Committing only the tracked diff as-is would ship Android builds with no Firebase config path at all, reproducing the `Error: unable to get Firebase Messaging instance` failure this story's own Completion Notes describe fixing. Staged (`git add`) alongside this diff. [apps/mobile/app.config.js:1]
- [x] [Review][Patch] `sprint-status.yaml` and `docs/decisions.md` in this diff mark Story 6.2 `done` and document its behavior (`private.send_push_notification`, `notification_dispatches`/`notification_deliveries`, the delivery processor), but the code that implements it — `supabase/migrations/0045_subscription_lifecycle_notifications.sql`, its two pgTAP files, and the `6-2`/`6-3` story markdown files — was untracked. Staged all six files (`git add`) so the commit's status/doc claims are backed by the code that implements them. [supabase/migrations/0045_subscription_lifecycle_notifications.sql:1]
- [x] [Review][Defer] `subscribeToPushTokenChanges`'s `.catch()` handler is unreachable dead code — `fetchAndStorePushToken` already swallows every error internally via its own try/catch, so the new `if (error) throw error` in `upsertPushToken` can never propagate to it. Pre-existing (the function already never rejected before this diff's throw was added one level down), not introduced by this change — deferred, pre-existing pattern. [apps/mobile/src/services/pushTokens.ts:93]

Dismissed as noise/non-issues (verified against repo state, not new problems):
- Deletion of `apps/mobile/src/services/pushTokens.test.ts` — already approved in Review round 1 (no `jest` dependency exists in `apps/mobile/package.json`; this re-applies a previously-checked-off fix, not a new regression).
- `_layout.tsx`'s `session` → `sessionUserId` effect-dependency narrowing — already approved in Review round 1 (`git show HEAD` confirms this fix was checked off but never actually committed until now).
- Theoretical empty-string `session.user.id` edge case — unreachable; Supabase Auth's `user.id` is always a populated UUID from the JWT `sub` claim.
- pgTAP DELETE-denial test rewrite (`has_table_privilege` check replacing a raw `DELETE`) — verified more correct against migration 0042's actual grants (`authenticated` is never granted `DELETE`); a genuine strengthening, not a deviation.
