...
- [x] [Review][Decision] `AnimatedSplashOverlay` (the JS component that takes over immediately after the native splash hides) still showed the old, unbranded Expo logo on the old brand background — undoing this story's own splash→app continuity goal on every single cold launch. `apps/mobile/src/components/animated-icon.tsx:38,109,145` (pre-fix) hardcoded `backgroundColor: Brand.primary` (`#1B2A41`) and `require('@/assets/images/expo-logo.png')` (the pre-story Expo "A" mark). This story changed the *native* splash background to `#0A0F17` (AC #1) and the *native* splash/icon marks to the new GymOS glyph (AC #2/#3), but never touched this JS overlay component, which was not in this story's declared "No `apps/mobile/src/` changes" scope. **Resolved by the user: fix both now.** `splashOverlay.backgroundColor` now reads `Colors.dark.background` (`#0A0F17`, same value as the native splash), and the overlay image now reuses the already-produced `assets/images/splash-icon.png` (background baked in, seams with the native splash) instead of `expo-logo.png`, sized `76×44` to match its 228×133 aspect ratio and `app.json`'s `imageWidth: 76`. `styles.image` (the unrelated, unused `AnimatedIcon` component's own logo, confirmed dead code — not imported anywhere in `src/`) was deliberately left untouched, since `splash-icon.png`'s baked-in rectangular background would look wrong inside `AnimatedIcon`'s circular gradient composition. Verified: `pnpm typecheck` clean (0 errors). Related, lower-severity observation from the same review pass, left for manual on-device QA: the rebranded Android/iOS icon marks (`android-icon-foreground.png`, `android-icon-monochrome.png`, `expo.icon/Assets/gymos-mark.png`) occupy noticeably less vertical space (~26% of canvas height) than the glyph they replaced (~41%), so the new mark may read as visually smaller inside a real launcher's icon mask.

- [x] [Review][Patch] AC #5(a)'s conditional "if `--platform ios` prebuild is available and runnable here, run it too" was never attempted — the Debug Log only mentions the `--platform android` run. Verified during this review: `npx expo prebuild --no-install --platform ios` **does** run cleanly in this (Linux) environment (structural pass, `✔ Finished prebuild`), closing the gap the AC left conditional on availability. Side effects (`ios/` directory, `package.json`'s `android`/`ios` script rewrite) were reverted the same way Task 7 already handles the Android run's equivalent side effect.

- [x] [Review][Defer] The ad-hoc `sharp`-based Node scripts used to produce every rebranded asset (`icon.png`, `splash-icon.png`, all three Android adaptive layers, `gymos-mark.png`) were never committed to the repo (per the Dev Agent Record's own Debug Log) — deferred, pre-existing pattern for one-off asset production in this project, not a defect in the shipped assets themselves. `apps/mobile/assets/expo.icon/Assets/gymos-mark.png` (and sibling regenerated assets)
...
- 2026-09-02: bmad-code-review — 3 parallel adversarial layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) against the isolated commit `66896f3` diff (baseline `69c348f`). Acceptance Auditor: zero AC violations, every claim independently re-verified against the repo. 14 raw findings, deduplicated to 9: 1 decision-needed (resolved by the user — see Review Findings), 1 patch applied (AC #5(a)'s conditional iOS prebuild, previously unattempted, now verified clean and logged), 1 deferred to `deferred-work.md` (uncommitted asset-generation scripts), 7 dismissed as noise (already-documented scope exclusions or speculative/no-impact observations — see Review Findings for the decision/patch detail). The decision finding was the significant one: `AnimatedSplashOverlay` (`apps/mobile/src/components/animated-icon.tsx`), the JS component that takes over immediately after the native splash this story rebrands, was still showing the old Expo logo on the old brand color — undoing the story's own splash→app continuity goal on every cold launch. Fixed: its background now matches the native splash's `#0A0F17`, and its image now reuses the already-produced `splash-icon.png` instead of `expo-logo.png`. `pnpm typecheck` clean. Status: review → done.
...
- **(2026-09-02, code review)** `npx expo prebuild --no-install --platform ios` — AC #5(a)'s conditional iOS prebuild check, not attempted during original implementation. Confirmed to run cleanly in this Linux devcontainer (`✔ Finished prebuild`), closing the gap. Same `package.json` script-rewrite side effect reverted; generated `ios/` directory removed.
