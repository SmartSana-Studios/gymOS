...
- [~] **Task 8: Manual verification** (AC: all) — **partially complete, blocked on browser access; see Dev Agent Record**
  - [x] Run against real local Supabase (WSL2 Docker — see `[[project_supabase_wsl]]`): (a) load `/messaging` as super_admin, confirm "not configured" state on a fresh DB; (b) save a valid instance ID, confirm it persists across a page reload and an `audit_log` row exists with correct old/new values; (c) attempt to save an empty/whitespace value, confirm inline error and no audit entry written — **all three verified via a real Supabase Auth → PostgREST → RPC network call (not just pgTAP), see Dev Agent Record; (a)/(b)/(c)'s actual page-rendering was not confirmed in-browser**.
  - [ ] (d) confirm Logout appears in the nav and signing out redirects to `/auth/login` — **not yet verified**: the Claude-in-Chrome browser tool's safety-check backend was unavailable this session (blocked navigation to both the local dev server and `https://example.com`), and the user's own browser login attempt at `http://127.0.0.1:3000/auth/login` hung and reloaded with no visible error despite the underlying Supabase Auth endpoint responding correctly (confirmed via direct `curl`, 0.13s, valid JWT with `app_role: super_admin`) — likely an environment-specific issue (stale WSL/Docker networking state, consistent with this machine's known WSL idle-shutdown quirk, `[[project_wsl_idle_shutdown_fix]]`). User is restarting their computer to retry.
  - [ ] (e) confirm a non-super-admin (e.g. a gym-owner dashboard session, or an unauthenticated request) cannot reach `/messaging`'s data — this should already be covered by the existing `(admin)/layout.tsx` guard (Story 1.8), not new logic this story adds, but verify it wasn't accidentally weakened. **Not yet independently re-verified this session** — code path is unchanged from Story 1.8 and pgTAP's own RLS deny-all coverage (Task 6) confirms a non-super-admin session gets 0 rows from `messaging_provider_config` directly.
...
- Task 8 is not fully complete — see Debug Log References above. AC #1-#3's data-layer behavior is verified; AC #1's page-rendering, AC #4, and AC #5 (Logout nav placement + redirect) still need an in-browser check once the environment issue clears. Story intentionally left at Status `in-progress`, not `review`, until Task 8 finishes — do not advance status without completing or explicitly re-scoping the remaining manual checks.
