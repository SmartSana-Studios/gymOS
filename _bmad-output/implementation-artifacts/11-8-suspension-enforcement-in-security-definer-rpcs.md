...
> **NFR-018 status after code review.** This story closes the `SECURITY DEFINER` half of AD-3 **for the 18 tables carrying `tenant_active_gate`**, and that part is verified. It does **not** make NFR-018 whole: the code review found that Epic 13's `workout_plans`, `workout_plan_exercises` and `workout_plan_completions` never received the gate policy at all, so a coach at a suspended gym can still author and hand off plans — through RLS *and* through Epic 13's own RPCs. That was a HIGH-severity item in `deferred-work.md`, and `suspension_rpc_coverage.test.sql` now pins the ungated-table set so it cannot grow silently.
>
> **Update 2026-09-09 — Story 11.9 closed that gap.** `0091_suspension_enforcement_for_workout_plans.sql` added `tenant_active_gate` to all three Epic 13 tables and the same `is distinct from 'active'` guard to `create_workout_plan()`, `update_workout_plan()` and `take_ownership_of_workout_plan()`, taking the gate to 21 tables and 21 guarded write-RPCs. **NFR-018 can now be claimed whole across the schema** — every `gym_id`-bearing table either carries the gate or is on the documented, test-pinned exclusion list (`exercise_library`, `gym_data_escalations`, `gym_payment_credentials`, `saas_billing_notices`, `saas_billing_payments`), and every `SECURITY DEFINER` function writing a gated table either carries the guard or is on 0090's justified exclusion list. Note that 11.9 also proved this story's own guardrail had a real blind spot: assertion 7's `least()` list omitted `update workout_plans`, so `take_ownership_of_workout_plan()`'s guard placement would have gone silently unverified — demonstrated by moving its guard below its write and watching the pre-11.9 assertion still report `ok`.
