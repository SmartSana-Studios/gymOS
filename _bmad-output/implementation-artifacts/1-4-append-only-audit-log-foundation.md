...
Status: review — implementation complete and verified: `supabase test db` run against local Docker Postgres, all 46 assertions pass (17 new + 29 pre-existing from Story 1.3, unaffected); `packages/types/src/database.ts` regenerated and confirmed to include `audit_log`/`log_audit_event()`
...
- `supabase/tests/audit_log_immutable.test.sql` (17 pgTAP assertions, **all passing**) covers: `log_audit_event()`'s system-caller path (null actor, caller-supplied label) and authenticated-session path (actor derived from `auth.uid()`/`public.users.display_name`, never caller-supplied); a regression case for a real session with no `display_name` set (guards the bug fixed above); a not-null-violation raise rather than a swallow for a missing `action_type`; and the grant-level append-only enforcement itself — critically, asserting `service_role` (which bypasses RLS in Supabase) is *still* blocked from UPDATE/DELETE by the grant-level `REVOKE`, which is the actual substance of AC #1 beyond what RLS alone could ever provide.
- **Executed and verified**: `supabase db reset` + `supabase test db` run against local Docker Postgres (via WSL). All 46 assertions pass across all 4 test files (17 new, 29 pre-existing from Story 1.3, unaffected). Two real bugs were caught only because the suite was actually run, not by static review — see Debug Log References for both.
- **Completed**: `packages/types/src/database.ts` regenerated via `supabase gen types typescript --local`; diff reviewed and confirmed to add exactly the `audit_log` table and `log_audit_event` function shapes, nothing else.
