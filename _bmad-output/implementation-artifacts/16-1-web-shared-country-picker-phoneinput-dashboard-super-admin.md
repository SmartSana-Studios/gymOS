...
- 2026-09-07: Closed the one remaining coverage gap (`RenewalModal`'s Mobile-Money branch) at user request ("seed fake data") by connecting a sandbox Tara Money account through the app's own real `connectPaymentProvider` Server Action/UI (Settings → Payment Account → Connect, fake `sandbox-fake-api-key-for-local-testing`/`SANDBOX-TEST-BUSINESS-001`/`sandbox-fake-webhook-secret-for-local-testing` values) rather than inserting into `gym_payment_credentials`/Vault directly — same "use the sanctioned path" precedent as every other local test-data convenience in this project's history. Re-verified live: `RenewalModal`'s "Mobile Money (Tara Money)" option now appears, its `PhoneInput` defaults to Cameroon with the member's on-file number pre-filled and correctly split into country + national number, the `tara-money` country list correctly excludes France and includes Ghana, and switching to Ghana updates the picker to `🇬🇭+233`. All five real `PhoneInput` call sites (`MemberModal`, `AddStaffModal`, `CreateGymModal`, `RenewalModal`, `PayNowButton`) are now confirmed working end-to-end in a real running app, not just by automated tests.
