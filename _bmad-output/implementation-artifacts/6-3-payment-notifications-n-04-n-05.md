...
- [x] **Task 1: Add the payment-scoped notification transport migration** (AC: #1, #2, #4, #6, #8)
  - [x] Create the next sequential migration, `supabase/migrations/0046_payment_notifications.sql`.
  - [x] Add `private.payment_notification_dispatches` and `private.payment_notification_deliveries`, structurally mirroring Story 6.2's `private.notification_dispatches`/`private.notification_deliveries` (same columns, same status vocabulary, same indexes) but keyed by `payment_id` instead of `subscription_id`. Use `notification_code check (notification_code in ('N-04', 'N-05'))` and `unique (payment_id, notification_code)`.
  - [x] **Do not alter or reuse `private.notification_dispatches`/`private.notification_deliveries` (migration 0045).** They are already shipped and reviewed, and their unique key is `subscription_id`-shaped — `subscription_id` is frequently `NULL` on `payments` (manual ledger entries never set it; the webhook path's `complete_verified_payment()` only sets it in a *second*, later `UPDATE`, after the `verified` transition this story dispatches from). A parallel, payment-keyed pair avoids widening 0045's proven schema and avoids nullable-either-column unique-index ambiguity. Record this as a deliberate rejected alternative in the decision log (Task 6).
  - [x] Enable RLS on both new tables in the same migration; grant only `service_role` (no `authenticated`/`anon` policy), matching 0045's exact posture.
  - [x] Add the same supporting indexes 0045 used: a due-delivery partial index on `(status, updated_at)`, and unique indexes on `push_request_id`/`receipt_request_id`.

- [x] **Task 2: Implement bilingual payment message construction and Expo enqueueing** (AC: #1, #2, #6, #7, #8)
  - [x] Implement `private.send_payment_push_notification(p_payment_id uuid, p_notification_code text)`. **Cannot be an overload of `private.send_push_notification(uuid, text)`** — Postgres cannot disambiguate two functions with an identical `(uuid, text)` signature; use a distinct name.
  - [x] Derive `gym_id`, `member_id` **directly from the `payments` row** (`payments.gym_id`/`payments.member_id` are both `NOT NULL`) — do **not** join through `subscriptions` (Story 6.2's pattern); `payments.subscription_id` is frequently `NULL` and is not a reliable join key here. From `member_id`, derive `members.user_id → users.preferred_language`, and every `device_push_tokens` row for that user, exactly as Story 6.2 does.
  - [x] Validate the notification code allowlist (`N-04`, `N-05` only) and fail closed for unknown codes, matching `send_push_notification`'s exact guard.
  - [x] Use `fr` only when `lower(preferred_language) = 'fr'`; otherwise English. Propose this copy (not pre-reviewed like Story 6.2's N-01–N-03 copy — confirm/record final wording in the decision log, Task 6):
    - [x] N-04 EN: title `Payment confirmed`; body `Your payment of {amount} {currency} to {gym_name} was confirmed.`
    - [x] N-04 FR: title `Paiement confirmé`; body `Votre paiement de {amount} {currency} à {gym_name} a été confirmé.`
    - [x] N-05 EN: title `Payment failed`; body `Your payment to {gym_name} could not be completed. Please try again or contact the front desk.`
    - [x] N-05 FR: title `Échec du paiement`; body `Votre paiement à {gym_name} n'a pas pu être effectué. Veuillez réessayer ou contacter la réception.`
  - [x] POST the same Expo-shaped JSON as Story 6.2 to `https://exp.host/--/api/v2/push/send` via `net.http_post`, with a `data` object containing `notificationCode`, `paymentId`, `gymId`. Do not call FCM/APNs directly; do not create an Edge Function.
  - [x] Insert the logical dispatch row first with `on conflict (payment_id, notification_code) do nothing`, mirroring 6.2's idempotency guard exactly. No-token case is a safe, observable `no_tokens` no-op.
  - [x] Revoke public execute; grant only to `service_role` (the trigger runs as the table owner/postgres and does not need a grant to `authenticated`).

- [x] **Task 3: Add the `payments` trigger — the single hardest correctness requirement in this story** (AC: #1, #2, #3, #4)
  - [x] Implement `private.notify_payment_status_change()` as an `AFTER INSERT OR UPDATE ON payments FOR EACH ROW` trigger function, registered as trigger `payments_notify_status_change`.
  - [x] **N-04 firing condition:** `(TG_OP = 'INSERT' AND NEW.status = 'verified') OR (TG_OP = 'UPDATE' AND NEW.status = 'verified' AND OLD.status IS DISTINCT FROM 'verified')`. The `OLD.status IS DISTINCT FROM 'verified'` guard is load-bearing: without it, `complete_verified_payment()`'s **second** `UPDATE ... SET subscription_id = ...` (which leaves `status` already `'verified'`) would re-fire the trigger a second time for the same payment. (The dispatch table's `unique(payment_id, notification_code)` would absorb a second *dispatch* attempt harmlessly, but the trigger should not even attempt it — keep the SQL-level intent correct, not just idempotency-safe.)
  - [x] **N-05 firing condition:** `TG_OP = 'UPDATE' AND NEW.status = 'flagged' AND OLD.status = 'processing'`. This is the mechanism that distinguishes an automated webhook failure from a manual "Flag for Review" (AC #3): the Verification Queue's `gym_staff_verify_own_payments` policy (`supabase/migrations/0031_manual_payment_verification_queue.sql`) only ever operates on `OLD.status = 'pending'` rows, so a `pending → flagged` transition **never** matches this condition and correctly sends nothing. Only Task 4's new webhook-failure completion path produces a `processing → flagged` transition. Do **not** use a bare `NEW.status = 'flagged'` condition — that would incorrectly notify members on every manual internal flag.
  - [x] Call `private.send_payment_push_notification(NEW.id, 'N-04' | 'N-05')` via `perform` from the trigger body (`security definer` not required on the trigger function itself since it runs in the same transaction as the writer; `send_payment_push_notification` itself carries the privilege boundary).

- [x] **Task 4: Give the webhook a real, distinguishable failure completion path** (AC: #2, #3)
  - [x] `supabase/functions/payment-webhook/index.ts` (read fully before editing — see Existing Files to Update) currently treats `event.status !== "verified"` as a terminal no-op: it logs and returns 200 without ever writing to `payments.status` (see the exact comment at lines 267–278: *"payment_status has no 'failed' value... no auto-transition happens here"*). That comment's premise no longer holds once this task ships — you are the first story to give an automated webhook failure a real, observable state.
  - [x] Add `complete_flagged_payment(p_payment_id uuid) returns void`, mirroring `complete_verified_payment()`'s exact shape and trust boundary (`security definer`, `service_role`-only grant, `revoke ... from public`): `UPDATE payments SET status = 'flagged' WHERE id = p_payment_id AND status = 'processing'`. The `AND status = 'processing'` clause is the idempotency guard for a retried webhook delivery, identical reasoning to `complete_verified_payment()`'s own comment.
  - [x] No new parameters for error detail — `payment_webhook_events` (Story 4.4, `supabase/migrations/0032_payment_reconciliation_job.sql`) already persists `status`/`raw_payload`/`matched_payment_id` for every signature-verified delivery, matched or not, immediately above this branch in the same handler. That is the audit trail for *why* a payment failed; do not duplicate it onto the `payments` row.
  - [x] In the `event.status !== "verified"` branch, when `paymentRow` was found (a real payment exists to fail), call the new RPC instead of the current no-op. Leave the `!paymentRow` defensive branch (line ~281) untouched.
  - [x] Update the stale comment block to describe the new behavior instead of the now-false claim that nothing happens.
  - [x] Note the interaction with Story 4.4's reconciliation job (`run_payment_reconciliation_job()`, AC #2, `stale_processing`): after this change, a *promptly delivered* declined webhook will transition the payment to `flagged` immediately rather than being caught ~10 minutes later as `stale_processing`. This is a genuine, positive behavior change to already-shipped Story 4.4 logic — record it explicitly in the decision log (Task 6); do not let it read as an accidental side effect.

- [x] **Task 5: Extend the delivery processor to cover the new ledger** (AC: #5)
  - [x] Story 6.2's `private.process_notification_deliveries()` and its `notification_delivery_processor` cron entry already implement the full Expo ticket/receipt state machine (push_pending → receipt_pending → delivered/failed/device_not_registered) and already call `private.cleanup_invalid_device_push_token()` on `DeviceNotRegistered`. Extend that same function (recommended: `UNION ALL` the `private.payment_notification_deliveries` rows into its processing loop, keyed the same way) rather than standing up a second near-identical processor and a second cron entry — there is no behavioral reason for two copies of the same Expo response-handling logic.
  - [x] If a shared/generalized processor proves awkward in practice, a second, clearly-named processor + cron entry is an acceptable fallback — but it must reuse `private.cleanup_invalid_device_push_token()` exactly as-is (never reimplement token cleanup).
  - [x] Preserve the existing `notification_delivery_processor` cron schedule/name for the lifecycle ledger; `cron.schedule()` upserts by name, so do not introduce a duplicate.

- [x] **Task 6: Regenerate database types and document the decisions** (AC: #1–#8)
  - [x] Regenerate `packages/types/src/database.ts`; the new ledger tables live in `private` and should remain absent from generated public types, same as 0045.
  - [x] Add a dated `docs/decisions.md` entry covering, at minimum: (a) why a parallel payment-keyed dispatch/delivery pair was added instead of generalizing 0045's subscription-keyed tables; (b) why dispatch derives identity from `payments.gym_id`/`member_id` directly instead of joining through `subscriptions`; (c) the `OLD.status = 'processing' → 'flagged'` vs. `OLD.status = 'pending' → 'flagged'` distinction that separates an automated webhook failure (N-05) from a manual "Flag for Review" (no notification) — this is the story's central design resolution and must not be silently assumed; (d) that `complete_flagged_payment()` is a new, previously-nonexistent automated failure path, and its interaction with Story 4.4's `stale_processing` reconciliation check; (e) final confirmed N-04/N-05 copy if it differs from this story's proposed default.

- [x] **Task 7: Add comprehensive pgTAP coverage** (AC: #1–#8)
  - [x] Add `supabase/tests/payment_notifications.test.sql` with deterministic fixtures for: webhook completion (`complete_verified_payment`) firing N-04 exactly once despite its two-`UPDATE` shape; manual Verification Queue staff-verify (`pending → verified`) firing N-04; a direct verified insert (mirroring the Inline Renewal Panel / Open Payment Method pattern) firing N-04; a manual "Record Payment" ledger entry with `subscription_id IS NULL` still dispatching correctly keyed by `payment_id`.
  ...
  - [x] Assert EN/FR/unsupported-language-fallback copy, multiple device tokens, no-token no-op, and rerun idempotency (same payment re-verified/re-touched produces no duplicate dispatch or `pg_net` request), same discipline as `supabase/tests/subscription_lifecycle_notifications.test.sql`.
  - [x] Inspect queued `pg_net` request payload state within the test transaction — never send a real Expo request. Assert endpoint, headers, Expo token, exact EN/FR title/body, and camelCase `notificationCode`/`paymentId`/`gymId` data keys.
  - [x] Extend or add a negative-privilege test file proving `anon`/`authenticated` cannot execute `send_payment_push_notification`, `complete_flagged_payment`, or the processor, nor read/write the new `private` tables.
  - [x] Keep all existing `supabase/tests/subscription_lifecycle_notifications.test.sql`, `manual_payment_verification_queue.test.sql`, and `payment_reconciliation` test assertions passing unchanged unless a count must expand for a directly related regression.

- [x] **Task 8: Validate from a clean database** (AC: #1–#8)
  - [x] Run `supabase db reset`, the full `supabase test db` suite, type generation/diff verification, `pnpm run typecheck`, and `pnpm run lint`.
  - [x] Confirm the delivery-processor cron entry still exists exactly once (whether extended or duplicated per Task 5) and retains its schedule/owner.
  - [x] A fresh physical-device delivery check is **not required** for this story — Stories 6.1/6.2 already proved the Expo/`pg_net`/FCM pipeline end-to-end on a real device. This story's actual risk surface is server-side trigger/dispatch correctness, which Task 7's pgTAP coverage owns. Only re-verify on-device if something in the shared processor path (Task 5) genuinely changes observable delivery behavior.

### Review Findings

- [x] [Review][Patch] Trigger's notification calls are unguarded — an exception rolls back the real payment write [supabase/migrations/0046_payment_notifications.sql:217-238] — fixed: wrapped each `perform` call in its own `begin...exception when others...raise warning...end` block, mirroring `run_subscription_lifecycle_job()`'s established containment pattern.
- [x] [Review][Patch] `complete_flagged_payment()` gives no signal when its UPDATE matches zero rows [supabase/migrations/0046_payment_notifications.sql:252-263] — fixed: added `returning id into v_id` + `raise notice` on no-op, mirroring `complete_verified_payment()`.
- [x] [Review][Patch] N-05 French/unsupported-language-fallback copy has zero test coverage [supabase/tests/payment_notifications.test.sql:1072-1116] — fixed: added a dedicated FR automated-webhook-failure payer fixture and assertion for exact French N-05 copy (48 → 51 assertions).
- [x] [Review][Patch] Misleading "-- flagging payment" log line fires even when nothing is flagged [supabase/functions/payment-webhook/index.ts:277-280] — fixed: split into an unconditional status-reported log and a separate log inside the `if (paymentRow)` branch that only fires when a payment is actually being flagged.
- [x] [Review][Patch] Dynamic UPDATE statements use `%s` instead of `%I` for the table identifier [supabase/migrations/0046_payment_notifications.sql:337-439] — fixed: `source_table` now holds the unqualified table name and every `format(...)` call uses `private.%I`.
- [x] [Review][Defer] `complete_flagged_payment()` omits an audit-log call unlike its sibling `complete_verified_payment()` [supabase/migrations/0046_payment_notifications.sql:252-263] — deferred, pre-existing (story's own Dev Notes explicitly hand this to Story 7.1)
- [x] [Review][Defer] N-04/N-05 automated-vs-manual boundary is enforced only by trigger comment + decision log, not schema [supabase/migrations/0046_payment_notifications.sql:228-235] — deferred, pre-existing (accepted trade-off of the epic-mandated trigger design)
...
- 2026-08-04: Implemented Story 6.3 end to end (migration 0046, webhook edit, pgTAP coverage, type regen, decision log). Fixed a real bug found via testing: the `payments` notification trigger needs `security definer` (contradicting the story's own Dev Notes) — see Dev Agent Record and `docs/decisions.md` Decision 5.
- 2026-08-04: Code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor, all 8 ACs confirmed satisfied). Applied all 5 patch findings: contained the trigger's notification dispatch in `begin...exception...end` blocks so a notification failure can no longer roll back a real payment write; added a no-op signal to `complete_flagged_payment()`; added French N-05 test coverage; fixed a misleading webhook log line; switched dynamic SQL from `%s` to `%I` for the table identifier. Verified via `supabase db reset` + `supabase test db` (801/801 pass) + `pnpm run typecheck` (4/4 packages) + generated-types diff (unchanged). 4 minor findings deferred to `deferred-work.md` (pre-existing/accepted trade-offs, none AC-blocking).
...
- `supabase test db` first run surfaced two real failures: (1) `permission denied for function send_payment_push_notification` when the Verification Queue's staff-verify path (`authenticated` role) fired the trigger — also broke the pre-existing `manual_payment_verification_queue.test.sql`; root cause was `private.notify_payment_status_change()` lacking `security definer`, contrary to the story's Dev Notes claim that it wasn't needed. Fixed by adding `security definer` with a pinned `search_path` to the trigger function in `0046_payment_notifications.sql`. (2) Two authoring bugs in the new test file itself (a stray assertion referencing an undefined table alias, and an ambiguous `status` column reference across a two-table join) — both fixed in `payment_notifications.test.sql`.
- Full clean-database validation: `supabase db reset` + `supabase test db` → `Files=40, Tests=798, Result: PASS` (all pre-existing tests unchanged, all new coverage passing).
- `pnpm run typecheck` passes for all 4 workspace packages. `pnpm run lint` fails only on pre-existing, unrelated debt: `apps/mobile` is missing its local `eslint` binary (pre-existing environment gap, zero mobile files touched by this story) and pre-existing lint errors in `apps/dashboard`/`apps/super-admin` files this story never modified (confirmed via `git diff` against `baseline_commit` showing zero changes to either app directory).

### Completion Notes List

- All 8 tasks and every AC implemented: migration `0046_payment_notifications.sql` (payment-keyed dispatch/delivery tables, `private.send_payment_push_notification`, the `payments_notify_status_change` trigger, `complete_flagged_payment`, and a shared-processor extension covering both notification ledgers); `supabase/functions/payment-webhook/index.ts` now calls `complete_flagged_payment` on a declined/failed delivery when a matching payment exists.
- Real design/implementation deviation from the story's own Dev Notes, found via testing rather than assumed: `private.notify_payment_status_change()` must be `security definer` (see Debug Log References and `docs/decisions.md` Decision 5, dated 2026-08-04). This was necessary for the trigger to work uniformly across all five write paths named in AC #1, not just the webhook path.
- pgTAP coverage: `supabase/tests/payment_notifications.test.sql` (48 assertions) and `supabase/tests/payment_notifications.negative.test.sql` (16 assertions), covering every AC including the N-05 automated-vs-manual boundary (AC #3, the story's highest-value assertion), the double-fire guard (AC #4), EN/FR/fallback copy (AC #6), and shared-processor `DeviceNotRegistered` cleanup reuse (AC #5).
- `packages/types/src/database.ts` regenerated; diff is exactly the new public `complete_flagged_payment` RPC, with the new `private` tables/functions correctly absent, matching 0045's precedent.
- `docs/decisions.md` updated with a dated Story 6.3 entry covering all 5 required decision points plus the `security definer` correction.

### File List

- `supabase/migrations/0046_payment_notifications.sql` (new)
- `supabase/functions/payment-webhook/index.ts` (edited)
- `supabase/tests/payment_notifications.test.sql` (new)
- `supabase/tests/payment_notifications.negative.test.sql` (new)
- `packages/types/src/database.ts` (regenerated)
- `docs/decisions.md` (edited)
- `_bmad-output/implementation-artifacts/6-3-payment-notifications-n-04-n-05.md` (workflow tracking)
