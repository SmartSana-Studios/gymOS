...
2. **New Server Action `sendMemberInvite(memberId: string)`** in `apps/dashboard/app/(dashboard)/members/actions.ts`, alongside the file's existing exports (`createMember`, `editMember`, etc. — same `"use server"` file, same `{ data, error }`-never-throws convention). Orchestration:
   ...
   - Return `{ data: { success: true } | { success: false }, error: null }` — **not** the `{data, error: AppError}` shape used for validation failures elsewhere in this file, because a send failure is an *expected* outcome the UI must render as AC #3's fallback state, not a generic error toast. Model it as `{ data: { sent: boolean }, error: AppError | null }` where `error` is only set for genuine failures (member not found, not authorized) and `sent: false` (with `error: null`) is the expected "gateway unreachable, show fallback" case — the client component branches on `sent` for AC #2 vs. AC #3, not on `error`. **Superseded by the 2026-08-13 code-review fix** (see Task 3 below): a genuine `error` (e.g. member not found) now shows the server's own message and does *not* open the fallback modal, since AC #3's fallback is scoped to gateway-reachability failures, not a not-found/authorization outcome.
...
- [x] **Task 3: `MembersPageClient.tsx` wiring** (AC: #1, #2, #3, #4)
  ...
  - [x] `sent: false` → `showToast(t("members.invite.sendFailedFallback"))` then `setInvitingMember(member)` (opens the existing fallback modal, unmodified). **Code-review fix (2026-08-13):** a genuine `error` (e.g. member not found) instead shows `error.message` directly and does *not* open the fallback modal — that outcome isn't a gateway-reachability failure, so AC #3's fallback doesn't apply to it.
...
**2026-08-14 code-review round (bmad-code-review, diff vs. baseline `1e6a099`):**

- [x] [Review][Patch] `sendEvolutionApiMessage`'s 10s timeout is cleared right after the initial `fetch()` resolves, before `response.text()` runs — if the server sends headers quickly then stalls mid-body, the abort no longer applies and the read can hang indefinitely [apps/dashboard/lib/messaging/EvolutionApiMessageProvider.ts:60-86]
- [x] [Review][Patch] `instance_id` (a DB value) is interpolated into the `fetch` URL without `encodeURIComponent`, a latent malformed-URL footgun if that value is ever loosened beyond Super-Admin-only writes [apps/dashboard/lib/messaging/EvolutionApiMessageProvider.ts:54]
- [x] [Review][Patch] A genuine `messaging_provider_config` query error (DB outage, RLS misconfig) and the legitimate "not configured" state both return the identical `"Evolution API instance is not configured"` message with no server-side log to distinguish them, misleading an operator debugging real infra failure [apps/dashboard/lib/messaging/EvolutionApiMessageProvider.ts:44]
- [x] [Review][Patch] `sendMemberInvite`'s `memberId` validation reuses `assignCoachSchema.shape.memberId` rather than its own validator, a hidden coupling where an unrelated future change to the coach-assignment schema could silently affect member-invite validation [apps/dashboard/app/(dashboard)/members/actions.ts:219]
- [x] [Review][Patch] `handleSendInvite`'s `finally` clears `sendingInviteId` unconditionally rather than only when it still matches the just-completed member — sending invites to two members concurrently lets the first request's completion clear the second row's still-in-flight indicator early, re-enabling its button and allowing a duplicate send before the second request actually finishes [apps/dashboard/app/(dashboard)/members/components/MembersPageClient.tsx:223]
- [x] [Review][Patch] `EvolutionApiMessageProvider.test.ts` has no test for the `AbortError`/timeout branch or the generic non-`Error`-thrown branch inside the `fetch` `catch` — the Senior Developer Review's own "8 cases covering every branch" claim (line 223 below) is inaccurate; the timeout path is also the one directly implicated by the hang bug noted above [apps/dashboard/lib/messaging/EvolutionApiMessageProvider.test.ts]
- [x] [Review][Patch] `MembersPageClient.sendInvite.test.tsx` never exercises `handleSendInvite`'s `catch` (rejected-promise) branch — all tests mock `sendMemberInvite` as resolving, none as rejecting [apps/dashboard/app/(dashboard)/members/components/MembersPageClient.sendInvite.test.tsx]
- [x] [Review][Patch] The "Senior Developer Review (AI)" Outcome below still reads "Approved... Status: review → done" despite the story being reopened `done` → `in-progress` afterward (see the first Review Finding above) — the verdict is stale and self-contradictory relative to the rest of this file [2-10-automated-member-invite-via-evolution-api.md:225-227]
- [x] [Review][Patch] The Change Log has no entry recording the `done` → `in-progress` reopen or the subsequent re-fix/re-review round — the reopening and its rationale exist only in this Review Findings section and in `sprint-status.yaml`'s banner comments [2-10-automated-member-invite-via-evolution-api.md, Change Log section]
- [x] [Review][Patch] Dev Notes/Task 3's text ("the client component branches on `sent` for AC #2 vs. AC #3, not on `error`") is out of sync with the shipped, deliberate error-branch split in `handleSendInvite` (confirmed intentional by the 2026-08-13 code-review fix and its own test's comment) — the story document doesn't reflect the behavior it now describes as done [2-10-automated-member-invite-via-evolution-api.md, Dev Notes §"What this story adds" / Task 3]
- [x] [Review][Defer] `sendMemberInvite` has no `try`/`catch` around `getMemberForInvite`/`getDashboardShellContext` — a rare `getClaims()` SDK throw would reject uncaught and break this file's documented `{data, error}` never-throws contract. The same unaddressed risk already exists via `Promise.all` in 5+ `page.tsx` routes across this app (`page.tsx`, `attendance/page.tsx`, `audit/page.tsx`, `members/page.tsx`, `payments/page.tsx`) — deferred, pre-existing, not something to patch in this one function without a broader app-wide decision [apps/dashboard/app/(dashboard)/members/actions.ts:226-234]
- [x] [Review][Defer] `handleSendInvite`'s async state updates (`setSendingInviteId`/`showToast`/`setInvitingMember`) aren't guarded against the component unmounting mid-request — a dev-only React warning in an edge case, matching this app's existing pattern elsewhere — deferred, pre-existing [apps/dashboard/app/(dashboard)/members/components/MembersPageClient.tsx:194-224]
...
- 2026-08-13: Reopened `done` → `in-progress` — undocumented "code review fix" changes found uncommitted against `actions.ts`, `MembersPageClient.tsx`, and `services/members.ts` (`Promise.all` parallelization in `sendMemberInvite`, error-vs-fallback branching split in `handleSendInvite`, `role`/`deactivated_at` filter added to `getMemberForInvite`), post-dating this story's `done` status with no corresponding record. Missing test coverage for the `role`/`deactivated_at` filter fixed same day (see Review Findings above); changes committed as `e61439b`.
- 2026-08-14: Code review (AI) found the `Promise.all` parallelization committed the day before was itself a regression — it reintroduced the unguarded-`getClaims()`-inside-`Promise.all` failure mode `services/session.ts`'s own Story 1.7 finding deliberately avoids, running `getDashboardShellContext()` even when `getMemberForInvite()` was guaranteed to fail fast. Reverted to sequential awaits, committed as `60856b6`.
- 2026-08-14: `bmad-code-review` full pass (diff vs. baseline `1e6a099`) — 10 Patch findings, all applied same day: a response-body read left unbounded by the fetch timeout once headers arrived (`EvolutionApiMessageProvider.ts`), unescaped `instance_id` URL interpolation, a DB query error conflated with the "not configured" state, `memberId` validation decoupled from `assignCoachSchema` into a new `memberIdSchema` (`packages/types`), a `sendingInviteId` race letting a completed invite's `finally` clear a different in-flight row's state, missing test coverage for the timeout/non-Error-catch branches and for `handleSendInvite`'s catch branch (3 new tests, 28/28 passing), plus this Change Log/Outcome/Dev-Notes sync. 2 Defer findings logged to `deferred-work.md` (an uncaught-throw risk shared with 5+ other routes; a cosmetic unmount-state-update warning). 9 findings dismissed as noise or false positives after verification.
...
**Approved at the time of this review (2026-08-13).** Status: review → done. Sprint status synced.

**Superseded 2026-08-13:** reopened `done` → `in-progress` after undocumented "code review fix" changes were found uncommitted on top of this already-`done` story (see Review Findings above) — this Outcome reflects the state of the code as it stood at review time, not the story's current status. See the Change Log for the full sequence.
